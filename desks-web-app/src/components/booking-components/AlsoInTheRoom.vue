<template>
  <v-alert v-if="bookings.length <= 0" class="mx-0">There are no other bookings for the office today.</v-alert>
  <v-list v-else>
    <v-list-item class="px-0 " two-line v-for="booking in bookings" :key="booking.id">
      <v-list-item-header>
        ...
        <v-list-item-subtitle>{{ $format.timeRange(booking.start, booking.end) }}
